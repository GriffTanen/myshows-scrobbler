...
Включается полем `auto_confirm: true` в `data/config.json` (по умолчанию `false` — функция выключена):
...
1. Откройте `chrome://extensions` (или аналог в другом Chromium-браузере), включите **Режим разработчика**.
2. **Загрузить распакованное расширение** → укажите папку [`extension/`](extension/) из этого репозитория.
