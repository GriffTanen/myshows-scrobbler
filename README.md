...
> **Это форк** [myshowsme/myshows-scrobbler](https://github.com/myshowsme/myshows-scrobbler) с тремя дополнениями поверх оригинала:
>
> - скрытый фильтр по пользователю для источника **Jellyfin** (аналог уже существующего Plex-фильтра) — см. [Фильтр пользователей Jellyfin](#фильтр-пользователей-jellyfin);
...
### Фильтр пользователей Jellyfin

То же самое для источника `jellyfin` (в этом форке; апстрим фильтрует только Plex) — на общем Jellyfin-сервере скробблятся сеансы всех пользователей, а не только ваши. Поле `user_filter` тоже задаётся только вручную в `data/config.json`, в интерфейсе его нет:
...
Засчитываются только сеансы, у которых `UserName` или `UserId` совпадает с записью из списка (без учёта регистра и пробелов по краям). Пустой или отсутствующий `user_filter` — засчитываются все зрители. **Ограничение**: `EmbyAdapter` наследует код Jellyfin-адаптера, но переопределяет получение сессий собственной копией без вызова фильтра — на источник `emby` `user_filter` пока не влияет.
