...
В том же прогоне переносятся и **оценки фильмов и сериалов** (в выбранном направлении). Шкалы отличаются масштабом — MyShows использует 0–5 с шагом в ползвезды, Jellyfin 0–10, — но это одна и та же 10-ступенчатая шкала со сдвигом ×2, поэтому оценка конвертируется точно в обе стороны (3,5★ ↔ 7/10). Предпросмотр отдельно показывает, сколько оценок будет перенесено.
