...
Переключатель направления в панели поддерживает и **обратный импорт: MyShows → Jellyfin** — отметить в Jellyfin как просмотренное то, что вы уже посмотрели на MyShows. Обратное направление работает **только для сериалов** (у MyShows нет API со списком просмотренных фильмов) и сопоставляет сериалы по IMDb id; запись в Jellyfin обратима. Тот же предпросмотр перед применением.
